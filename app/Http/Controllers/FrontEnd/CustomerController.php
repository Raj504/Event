...
class CustomerController extends Controller
{
  ...
  {
      Log::info('Starting user registration process.', ['request' => $request->all()]);
      ...
      Log::info('Verification email initiated.', ['email' => $request->email]);
  
      Customer::create($in);
  
      Log::info('User registered successfully.', ['user' => $in]);
  ...
  {
      Log::info('Starting sendVerificationMail method.', [
          'email' => $request->email,
          'token' => $token
      ]);
      ...
      if (!$mailTemplate) {
          Log::error('Mail template not found for verify_email.');
          Session::flash('error', 'Mail template not found.');
          return;
      }
      ...
      if (!$info) {
          Log::error('Basic settings not found in the database.');
          Session::flash('error', 'SMTP settings missing.');
          return;
      }
      ...
      if ($info->smtp_status == 1) {
          ...
          Log::info('SMTP settings applied.', [
              'host' => $info->smtp_host,
              'port' => $info->smtp_port,
              'encryption' => $info->encryption
          ]);
      } else {
          Log::info('SMTP is disabled, using PHP mail function.');
      }
  ...
  {
      Log::info('Verifying signup token.', ['token' => $token]);
      ...
      try {
          ...
          Log::info('Email verified successfully for user.', ['user_id' => $customer->id]);
