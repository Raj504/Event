...
class CheckOutController extends Controller
{
  ...
  //   $online_gateways = OnlineGateway::where('status', 1)->get();      //done by raj
  //   $offline_gateways = OfflineGateway::where('status', 1)->orderBy('serial_number', 'asc')->get();     //done by raj 
  ...
  public function checkout2(Request $request)
  {
   // dd($request->all());
    ...
    if ($event_guest_checkout_status != 1 && !Auth::guard('customer')->user()) {
      return redirect()->route('customer.login', ['redirectPath' => 'event_checkout']);
    }
    ...
    if ($event_type->event_type == 'venue') {
      ...
      if ($request->pricing_type == 'free') {
        $select = true;
      } elseif ($request->pricing_type == 'normal' && $request->quantity > 0) {
        $select = true;
      } else {
        foreach ($request->quantity as $qty) {
          if ($qty > 0) {
    ...
    $information = [
      'selTickets' => '',
      'veg_guests' => $request->veg_guests ?? 0,
      'non_veg_guests' => $request->non_veg_guests ?? 0,
    ];

    // Store `vag` and `non_vag` in the session
    Session::put('veg_guests', $information['veg_guests']);
    Session::put('non_veg_guests', $information['non_veg_guests']);
    ...
    if ($event->event_type == 'online') {
      ...
      if ($request->pricing_type == 'normal') {
        $price = Ticket::where('event_id', $request->event_id)
          ->select(
            'price',
            'early_bird_discount',
        ...
        if ($price->early_bird_discount == 'enable') {
          ...
          $today = Carbon::now();
          if ($today <= $end) {
            $early_bird_dicount = ($price->early_bird_discount_type == 'fixed')
              ? $price->early_bird_discount_amount
              : ($price->early_bird_discount_amount * $total) / 100;
          } else {
            $early_bird_dicount = 0;
          }
        } else {
      ...
      foreach ($tickets as $ticket) {
        if ($ticket->pricing_type == 'variation') {
          ...
          foreach ($varArr1 as $key => $var1) {
            $var1['type'] = $ticket->pricing_type;
            $var1['early_bird_dicount'] = $ticket->early_bird_discount == 'enable' ? $ticket->early_bird_discount_amount : 0;
            $var1['ticket_id'] = $ticket->id;
            $ticketArr[] = $var1;
          }
        } else {
          $ticketArr[] = [
            'ticket_id' => $ticket->id,
            'early_bird_dicount' => $ticket->early_bird_discount == 'enable' ? $ticket->early_bird_discount_amount : 0,
            'name' => $ticket->title,
            'price' => $ticket->price,
            'type' => $ticket->pricing_type
          ];
      ...
      $sub_total = array_reduce($selTickets, function ($carry, $ticket) {
        return $carry + ($ticket['price'] * $ticket['qty']);
      }, 0);

      $total_early_bird_dicount = array_reduce($selTickets, function ($carry, $ticket) {
        return $carry + $ticket['early_bird_dicount'];
      }, 0);
      ...
      Session::put('total', round($total, 2));
      Session::put('sub_total', round($sub_total, 2));
      Session::put('quantity', array_sum(array_column($selTickets, 'qty')));
      Session::put('selTickets', $selTickets);
      Session::put('total_early_bird_dicount', round($total_early_bird_dicount, 2));
    }
  ...
  public function checkout()
  {
   ...
   // dd($stripe);
    ...
    //dd($information);
    //dd(Session::get('online_gateways'));
