...
class BookingController extends Controller
{
  ...
  public function generate_uuid(): string {
      // Generate a random UUID based on the RFC 4122 standard
    ...
    if ($request->total != 0 || Session::get('sub_total') != 0) {
      ...
      try {
        ...
        $arrData = array(
          ...
          'emi_amount' => $emiAmount //done by raj
        );
        ...
        Log::error('Booking creation failed: ' . $th->getMessage());
    }
    }
  }
  ...
  public function storeData($info)
  {
    try {
      ...
      $totalPaid = $info['emi_amount'] ?? round($info['price'], 2);

      $paymentStatus = $totalPaid == round($info['price'], 2) ? 'completed' : 'pending';
      ...
      $booking = Booking::create([
        ...
        'emi_amount' => array_key_exists('emi_amount', $info) ? round($info['emi_amount'], 2) : null, 
        // 'total_paid' => array_key_exists('total_paid', $info) ? round($info['total_paid'], 2) : null, 
        'total_paid' => $info['emi_amount'] ?? round($info['price'], 2), // Done ny raj
...
//done by raj
        if (isset($info['emi_amount'])) {
          $booking->total_paid += round($info['emi_amount'], 2);
          $booking->paymentStatus = $booking->total_paid == round($info['price'], 2) ? 'completed' : 'pending';
          $booking->save();
        }
  ...
  public function generateInvoice($bookingInfo, $eventId)
  {
    try {
      ...
      $emiAmount = $bookingInfo->emi_amount; // raj
      ...
      PDF::loadView('frontend.event.invoice', compact('bookingInfo', 'eventInfo', 'emiAmount', 'width', 'float', 'mb', 'ml'))->save($fileLocated);
