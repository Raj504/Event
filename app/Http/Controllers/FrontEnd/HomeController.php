...
class HomeController extends Controller
{
  ...
  {
      // Retrieve the first EventContent record to get the book_date
      $eventContent = EventContent::first();
  
      // Set the booked date if the event content exists
      $bookedDate = $eventContent ? Carbon::parse($eventContent->book_date)->format('Y-m-d') : null;
  
      // Return the view with only the bookedDate
      return view('frontend.home.index-v1', compact('bookedDate'));
  }
