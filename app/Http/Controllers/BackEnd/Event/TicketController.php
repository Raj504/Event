...
class TicketController extends Controller
{
  ...
  //   if (!$request->has(['language', 'event_id'])) {
  //     return redirect()->back()->withErrors(['Both language and event_id are required.']);
  // }
    $information = [];
  ...
  public function store(TicketRequest $request)
{
    try {
        ...
        if ($request->pricing_type_2 == 'free') {
            ...
            Log::info('Ticket created for free pricing', ['ticket_id' => $ticket->id]);
        } elseif ($request->pricing_type_2 == 'normal') {
            $in['pricing_type'] = 'normal';
            $in['price'] = $request->price;
            $in['f_price'] = $request->price;
            $ticket = Ticket::create($in);
            Log::info('Ticket created for normal pricing', ['ticket_id' => $ticket->id]);
            ...
            $ticket = Ticket::create($in);
            Log::info('Ticket created for variation pricing', ['ticket_id' => $ticket->id]);
            ...
            if ($request->pricing_type_2 == 'variation') {
                ...
                foreach ($languages as $language) {
                    ...
                    foreach ($variation_datas as $key => $data) {
                        ...
                        Log::info('Variation content created', ['variation_data' => $variations_data]);
                    }
                }
            }
        }

        // Insert ticket content for each language
        $languages = Language::all();
        foreach ($languages as $language) {
            $data = [
                'language_id' => $language->id,
            ...
            Log::info('Ticket content created for language', ['language_id' => $language->id, 'ticket_id' => $ticket->id]);
        }

        // Log success message
        Log::info('Ticket successfully created', ['ticket_id' => $ticket->id]);
        ...
        Log::error('Error occurred while creating ticket', ['error' => $e->getMessage()]);
