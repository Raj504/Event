    ...
    <section class="event-details-section pt-110 rpt-90 pb-90 rpb-70">
        <div class="container">
            <div class="event-details-content">
                ...
                <d iv class="event-details-image mb-50">
                    ...
                    <div class="sidebar-sticky">
                        <form action="{{ route('check-out2') }}" method="post"
                            ...
                            <div class="event-details-information">
                                ...
                                {{-- Show selected check-in and check-out dates --}}
                                <!-- @if(request('checkin_date') || request('checkout_date'))
                                        <div class="selected-dates">
                                            <p><strong>{{ __('Selected Check-In Date:') }}</strong> 
                                                {{ request('checkin_date') ? \Carbon\Carbon::parse(request('checkin_date'))->format('d M Y') : __('Not selected') }}
                                            </p>
                                            <p><strong>{{ __('Selected Check-Out Date:') }}</strong> 
                                                {{ request('checkout_date') ? \Carbon\Carbon::parse(request('checkout_date'))->format('d M Y') : __('Not selected') }}
                                            </p>
                                        </div>
                                    @else
                                        <p class="text-danger">{{ __('No dates selected.') }}</p>
                                    @endif -->
                                    <script>
                                        function updateSelectedDates() {
                                            // Get the values of the check-in and check-out inputs
                                            const checkinDate = document.getElementById('checkin').value;
                                            const checkoutDate = document.getElementById('checkout').value;

                                            // Select the div where the selected dates will be displayed
                                            ...
                                            if (checkinDate && checkoutDate) {
                                                // Update the div with the selected dates
                                                selectedDatesDiv.innerHTML = `
                                                    <p><strong>{{ __('Selected Check-In Date:') }}</strong> ${checkinDate}</p>
                                                    <p><strong>{{ __('Selected Check-Out Date:') }}</strong> ${checkoutDate}</p>
                                                `;
                                            } else {
                                                // Show a message when no dates are selected
                                                selectedDatesDiv.innerHTML = `<p class="text-danger">{{ __('No dates selected.') }}</p>`;
                                            }
                                        }

                                        // Attach the updateSelectedDates function to the onchange event of the date inputs
                                        document.getElementById('checkin').addEventListener('change', updateSelectedDates);
                                        document.getElementById('checkout').addEventListener('change', updateSelectedDates);
                                    </script>
                               ...
                               {{-- @php
                                    dd($content->book_date, request('checkin_date'), request('checkout_date'));
                                @endphp   --}}
                                
                                
                                <div class="mt-2">
                                    @php
                                        // Parse check-in, check-out, and booking dates
                                    ...
                                    @if($checkinDate && $checkoutDate && $bookingDate->between($checkinDate, $checkoutDate))
                                        <p class="text-danger"><strong>{{ __('Unavailable') }}</strong></p>
                                    @else
                                        <strong>{{ __('Booking Date:') }}</strong>
                                        <span>{{ FullDateTime($content->book_date) }}</span>
                                    @endif
                                </div>
                                ...
                                <!-- @if ($content->date_type == 'single' && $content->countdown_status == 1)
                                    <div class="event-details-top">
                                        <p>{{ __('The event is available for booking. Please select your date.') }}</p> -->
                                        <!-- @if ($startDateTime >= $now_time)
...
@else
    ...
    @endif -->
                            ...
                            <div class="features pt-4">
                                <h5>{{ __('Features') }}</h5>
                                @php
                                    // Decode features into an array if it is a JSON string
                                    $featuresArray = is_string($content->features)
                                        ? json_decode($content->features, true)
                                        : $content->features;
                                @endphp
                        ...
                        @endif
                                        ...
                                        <div class="additional-info pt-4">
                                            <h5>{{ __('Additional Information') }}</h5>
                                            <!-- <div class="mx-4">
                               <p><strong>{{ __('Hall Capacity') }}:</strong> {{ $event_content->hall_capacity ?? 'Not specified' }}</p>
                               <p><strong>{{ __('Veg Guests') }}:</strong>
                                   {{ $event_content->veg_guests ?? 'Not specified' }}</p>
                               <p><strong>{{ __('Non-Veg Guests') }}:</strong>
                                   {{ $event_content->non_veg_guests ?? 'Not specified' }}</p>
                               </div> -->
                                            <div class="">
                                                <label for="veg_guests"><strong>{{ __('Veg Guests') }}:</strong></label>
                                                <input type="number" id="veg_guests" name="veg_guests"
                                                    class="form-control"
                                                    value="{{ old('veg_guests', $event_content->veg_guests ?? '') }}"
                                                    placeholder="Enter Veg Guests" />
                                                <label
                                                    for="non_veg_guests"><strong>{{ __('Non-Veg Guests') }}:</strong></label>
                                                <input type="number" id="non_veg_guests" name="non_veg_guests"
                                                    class="form-control"
                                                    value="{{ old('non_veg_guests', $event_content->non_veg_guests ?? '') }}"
                                                    placeholder="Enter Non-Veg Guests" />
                                            </div>
                                        </div>
                                            ...
                                            @if (count($tickets) > 0)
                                                ...
                                                @foreach ($tickets as $ticket)
                                                    @if ($ticket->pricing_type == 'normal')
                                                        ...
                                                        @endphp
                                                        <p class="mb-0"><strong>{{ @$ticket_content->title }}</strong>
                                                        </p>
                                                    ...
                                                    @elseif($ticket->pricing_type == 'variation')
                                                        @php
                                                            ...
                                                            $de_lang = App\Models\Language::where(
                                                                'is_default',
                                                                1,
                                                            )->first();
        ...
        @if (count($related_events) > 0)
            ...
            <div class="related-event-wrap">
                @foreach ($related_events->take(3) as $event)
                                                    ...
                                                    <span class="price">
                                                        @if ($ticket->early_bird_discount == 'enable')
                                                            ...
                                                            @if ($ticket->early_bird_discount_type == 'fixed' && !$discount_date->isPast())
                                                                @php
                                                                    $calculate_price =
                                                                        $price - $ticket->early_bird_discount_amount;
                                                                @endphp
                                                                {{ symbolPrice($calculate_price) }}
                                                                <span><del>
                                                                        {{ symbolPrice($price) }}
