                                            ...
                                            <tbody>
                                                @foreach ($bookings as $item)
                                                    ...
                                                    @if (!empty($event))
                                                        <tr>
                                                            <td>
                                                                <a target="_blank" href="{{ route('event.details', ['slug' => $event->slug, 'id' => $event->event_id]) }}">
                                                                    {{ strlen($event->title) > 30 ? mb_substr($event->title, 0, 30) . '...' : $event->title }}
                                                                    @if (!is_null($item->emi_amount))
                                                                        <span class="badge badge-info">{{ __('EMI') }}</span>  
                                                                    @endif
                                                                </a>
                                                            </td>
                                                            <td>
                                                                @if ($item->organizer)
                                                                    <a target="_blank" href="{{ route('frontend.organizer.details', [$item->organizer->id, str_replace(' ', '-', $item->organizer->username)]) }}">
                                                            ...
                                                            <td>
                                                                {{ \Carbon\Carbon::parse($item->created_at)->translatedFormat('D, M d, Y h:i a') }}
                                                            </td>
                                                            <td>
                                                                @if ($item->paymentStatus=='completed') 
                                                                    <!-- If EMI Amount exists, payment was done using EMI -->
                                                                    <span class="badge badge-warning">{{ __('Paid') }}</span>
                                                                @else
                                                                    <a href="{{ url('pay/v1/secure/emi/'.$item->uuid) }}" class="btn btn-primary">Pay Now -></a>
                                                                @endif
                                                            </td>
                                                            <td>
