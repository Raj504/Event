                    ...
                    <div class="row">
                      ...
                      <div class="col-md-4">
                        <div class="payment-information">
                          ...
                          @if ($booking->emi_amount != null   && $booking->emi_amount > 0)
                            <p><b>{{ __('EMI Amount Paid') . ' : ' }} </b>
                                <span class="amount">{{ $booking->currencySymbol }} {{ $booking->emi_amount }}</span>
                            </p>

                            @php
                                // Calculate the pending amount
                                $pendingAmount = ($booking->price + $booking->tax) - $booking->emi_amount;
                            @endphp

                            <p><b>{{ __('Pending Amount') . ' : ' }}</b> 
                                <span class="amount">{{ $booking->currencySymbol }} {{ $pendingAmount }}</span>
                            </p>
                        @else
                            @php
                                // If payment is completed without EMI, pending amount should be 0
                                $pendingAmount = $booking->paymentStatus == 'completed' ? 0 : ($booking->price + $booking->tax);
                            @endphp
