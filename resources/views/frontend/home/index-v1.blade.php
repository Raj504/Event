...
<link rel="stylesheet" href="https://code.jquery.com/ui/1.14.0/jquery-ui.js">
...
@section('hero-section')
    ...
    <div class="hero-content">
      ...
      <form id="event-search" class="event-search mt-35" name="event-search" action="{{ route('events') }}" method="get">
        ...
        <div class="search-item">
          ...
          <input type="search" id="search" name="search-input" placeholder="{{ __('Search Location') }}">
        </div> 
        
        <div class="search-item">
        <label for="checkin"><i class="fas fa-calendar-alt"></i></label>
        <input type="date" id="checkin" name="checkin_date" value="{{ request('checkin_date') }}" placeholder="Check-in Date">
          </div>
          <div class="search-item">
              <label for="checkout"><i class="fas fa-calendar-alt"></i></label>
              <input type="date" id="checkout" name="checkout_date" value="{{ request('checkout_date') }}" placeholder="Check-out Date">
          </div>
  ...
  <script>
    // Ensure the booked date is formatted correctly for JavaScript
    const bookedDate = "{{ $bookedDate ?? '' }}"; // should be in 'YYYY-MM-DD'

    // Event listener for check-in date selection
    document.getElementById('checkin').addEventListener('change', function () {
        const checkinDate = this.value; // Date selected by user

        // Check if the selected check-in date matches the booked date
        if (checkinDate === bookedDate) {
            alert("This date is unavailable for booking.");
            this.value = ''; // Clear the input if the date is unavailable
        }
    });
</script> 
  ...
  <!-- <script>
    $(document).ready(function() {
        // Initialize the date picker for a single date selection
        $("#date").datepicker({
            dateFormat: "dd/mm/yy"
        });
    });
