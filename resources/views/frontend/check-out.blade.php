		...
		<div class="row">
			<div class="col-lg-8">
				...
				<div class="row mt-35">
					...
					<div class="col-sm-6">
						<div class="form-group">
							<label for="state">{{ __('State') }}</label>
							<select name="state" class="form-control form-control-new">
								<option value="">{{ __('Select State') }}</option>
								@foreach ($states as $state)
								<option value="{{ $state }}"
								{{ old('state', $authUser?->state) == $state ? 'selected' : '' }}>
								{{ $state }}
								</option>
								@endforeach
							</select>
			...
			<div class="col-lg-4">
				...
				<div>
					...
					<h5>Guest Details</h5>
					--}}
					<p><strong>Veg Guests:</strong> {{ session('veg_guests') }}</p>
					<p><strong>Non-Veg Guests:</strong> {{ session('non_veg_guests') }}</p>
				</div>
                ...
                <input type="hidden" name="symbol_total" value="{{ $symbol_total }}" >
				...
				<div class="form-group">
					...
					<!-- done by Raj -->
					<div id="emi-options" class="d-none mt-3">
						<div class="form-group">
							<label for="emi-duration">{{ __('EMI Duration (Months)') }}</label>
							<select id="emi-duration" name="emi_duration" class="form-control">
								<option value="3">{{ __('3 months') }}</option>
								<option value="6">{{ __('6 months') }}</option>
								<option value="12">{{ __('12 months') }}</option>
								<option value="18">{{ __('18 months') }}</option>
								<option value="24">{{ __('24 months') }}</option>
							</select>
						</div>
						<div class="form-group">
							<label for="emi-amount">{{ __('EMI Amount') }}</label>
							<input type="text" id="emi-amount" name="emi_amount" class="form-control" readonly placeholder="Calculated EMI Amount">
						</div>
						<!-- Hidden Field for Gateway -->
						<input type="hidden" id="emi-payment-method" name="gateway" value="emi">
					</div>
						{{-- @php
							dd($online_gateways);
						@endphp --}}
...
<script>
	document.addEventListener('DOMContentLoaded', function () {
	    // Parse grand total and tax amount safely
	    var grandTotal = parseFloat(document.getElementById('grand-total').value) || 0;
	    var taxAmount = parseFloat("{{ $tax }}") || 0;
	
	    // Show EMI options when the EMI button is clicked
	    var emiButton = document.getElementById('emi-button');
	    var emiOptions = document.getElementById('emi-options');
	
	    emiButton.addEventListener('click', function () {
	        emiOptions.classList.remove('d-none'); // Show EMI options
	    });
	
	    // Calculate EMI when duration changes
	    var emiDuration = document.getElementById('emi-duration');
	    var emiAmountField = document.getElementById('emi-amount');
	
	    emiDuration.addEventListener('change', function () {
	        var duration = parseInt(this.value);
	        if (duration > 0) {
	            var emiAmount = (grandTotal + taxAmount) / duration;
	            emiAmountField.value = emiAmount.toFixed(2); // Set EMI amount
	        } else {
	            emiAmountField.value = ''; // Clear the field if invalid duration
	        }
	    });
	});
</script>