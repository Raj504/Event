                ...
                <div class="row ">
                  ...
                  @if ($event->event_type == 'venue')
                    ...
                    @foreach($languages as $language)
                      <div class="col-lg-4">
                          <div class="form-group">
                              <label for="date_{{ $language->code }}">{{ __('Booking Date') }} ({{ $language->name }})</label>
                              <input type="date" 
                                    name="{{ $language->code }}book_date" 
                                    value="{{ old($language->code . 'book_date', $event->event_content->where('language_id', $language->id)->first()?->book_date ?? '') }}" 
                                    class="form-control" 
                                    id="date_{{ $language->code }}" 
                                    required>
                          </div>
                      </div>
                  @endforeach
    ...
    <div class="col">
        <div class="form-group {{ $language->direction == 1 ? 'rtl text-right' : '' }}">
            ...
            <div id="featuresContainer">
                @if(isset($event_content->features) && is_array($event_content->features))
                    @foreach($event_content->features as $feature)
                        @if($feature) <!-- Check to avoid null values -->
                            <div class="mb-2 feature-input d-flex"> <!-- Flexbox layout -->
                                <input type="text" class="form-control me-2" name="{{ $language->code }}_features[]" placeholder="Enter Event Feature" value="{{ $feature }}" />
                                <button type="button" class="btn btn-danger btn-sm removeFeatureBtn">Remove</button> <!-- Positioned after input -->
                            </div>
                        @endif
                    @endforeach
                @endif
                <div class="mb-2 feature-input d-flex">
                    <input type="text" class="form-control me-2" name="{{ $language->code }}_features[]" placeholder="Enter Event Feature" />
                    <button type="button" class="btn btn-danger btn-sm removeFeatureBtn">Remove</button> <!-- Positioned after input -->
                </div>
            </div>
            <button type="button" class="btn btn-primary mt-2" id="addFeatureBtn">Add More</button>
        </div>

        <!-- <div class="form-group mt-4">
        <label for="hallCapacity">{{ __('Hall Capacity') }}</label>
        <input type="number" class="form-control" id="hallCapacity" name="{{ $language->code }}_hall_capacity" placeholder="Enter Hall Capacity" value="{{ $event_content->hall_capacity ?? '' }}">
    </div>
    <div class="form-group mt-2">
        <label for="vegGuests">{{ __('Veg Guests') }}</label>
        <input type="number" class="form-control" id="vegGuests" name="{{ $language->code }}_veg_guests" placeholder="Enter Number of Veg Guests" value="{{ $event_content->veg_guests ?? '' }}">
    </div>
    <div class="form-group mt-2">
        <label for="nonVegGuests">{{ __('Non-Veg Guests') }}</label>
        <input type="number" class="form-control" id="nonVegGuests" name="{{ $language->code }}_non_veg_guests" placeholder="Enter Number of Non-Veg Guests" value="{{ $event_content->non_veg_guests ?? '' }}">
    </div> -->
    </div>
</div>
...
<script>
    document.getElementById('addFeatureBtn').addEventListener('click', function() {
        // Create a new div to contain the new input
        var newDiv = document.createElement('div');
        newDiv.className = 'mb-2 feature-input d-flex'; // Flexbox layout

        // Create a new input element
        var newInput = document.createElement('input');
        newInput.type = 'text';
        newInput.className = 'form-control me-2'; // Add margin end
        newInput.name = '{{ $language->code }}_features[]'; // Ensure it's an array for form submission
        newInput.placeholder = 'Enter Event Feature';

        // Create a remove button
        var removeButton = document.createElement('button');
        removeButton.type = 'button';
        removeButton.className = 'btn btn-danger btn-sm removeFeatureBtn'; // Class for styling
        ...
        // Append the new div to the featuresContainer
        document.getElementById('featuresContainer').appendChild(newDiv);
    });
