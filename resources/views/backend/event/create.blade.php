              ...
              <form id="eventForm" action="{{ route('admin.event_management.store_event') }}" method="POST"
                ...
                <div class="row ">
                  ...
                  @foreach($languages as $language)
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label for="date">{{ __('Booking Date') }} ({{ $language->name }})</label>
                            <input type="date" name="{{ $language->code }}book_date" value="{{ old($language->code . 'book_date') }}" class="form-control" id="date" required>
                            @error($language->code . 'book_date')
